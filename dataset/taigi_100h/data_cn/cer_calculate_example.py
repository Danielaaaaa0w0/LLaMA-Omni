...
def calculate_cer_demo():
    ...
    CER 公式: CER = (S + D + I) / N
        S = Substitutions (替換錯誤的字數)
        D = Deletions (刪掉應該有的字數)
        I = Insertions (多插入不該有的字數)
        N = 參考答案(Reference) 的總字元數
    """
    ...
    # ===== 方法一：全部字元合併計算 =====
    # ref_texts和pred_texts都是一個char list，process_characters會處理並算出S, I, D, hits等數據。
    ...
    for idx, cer_val in enumerate(cer_per_sentence, 1):
